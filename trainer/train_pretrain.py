...
from model.model import ZeusMindConfig
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        with autocast_ctx:
            ...
            # loss = (res.loss + res.aux_loss)  # ！修正：原手动计算loss_fct+loss_mask，现用模型内置的loss
            loss = res.loss
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ZeusMind Pretraining")
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="dataset/minimind_dataset/pretrain_t2t.jsonl",
        help="预训练数据路径",
    )
    parser.add_argument(
        "--from_weight",
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="ZeusMind-Pretrain", help="wandb项目名"
    )
    ...
    if args.use_wandb and is_main_process():
        ...
        # 构建实验名称，包含关键超参数
        wandb_run_name = f"ZeusMind-Pretrain-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
        wandb.init(
